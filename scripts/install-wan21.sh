...
if command -v huggingface-cli &>/dev/null; then
    ...
    echo -e "${RED}huggingface-cli was installed but is not available in PATH.${NC}"
    echo -e "${YELLOW}Restart the shell or run: $PYTHON_CMD -m huggingface_hub.commands.huggingface_cli download Wan-AI/Wan2.1-T2V-1.3B --local-dir $MODEL_PATH${NC}"
    exit 1
fi
